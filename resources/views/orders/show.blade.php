...
<div class="container p-3 m-3 mb-5 w-100 border border-dark bg-gradient bg-secondary text-dark">
  <div class="d-grid gap-2 col-6 mx-auto my-3">
    <h3 class="text-center col">Pedido da mesa "{{$order->table_number}}"</h3>
  </div>
  ...
  <div class="content-justify-right mt-2 p-1 w-25">
    <form class="" action="{{route('OrderProductStore', $order->id)}}" method="POST">
      @csrf
      <div class="d-flex">
      ...
      <tbody class="table-group-divider" id="tablecardapios">
        @foreach ($order->products as $product)
          <tr>
            ...
            <td>
              <form class="" action="{{route('OrderProductDestroy', [$order->id, $product->id])}}" method="POST">
                @csrf
                @method('delete')
